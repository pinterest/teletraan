...
_DEPLOY_STAGE_TRANSITIONS = dict(map(lambda i: (i, i+1), range(DeployStage.PRE_DOWNLOAD, DeployStage.POST_RESTART)))
...
class ServerlessClient(object):
    ...
    """
    def __init__(self, env_name, stage, build):
    ...
    def send_reports(self, env_reports=None):
        ...
        for report in reports:
            ...
            log.debug('%s -> %s' % (reports, ping_response))
    ...
    def _create_response(self, report):
        ...
        if report.deployId is None or report.deployId != self._deploy_id:
            # first report from agent, start first deploy stage.
            return _new_response_value(DeployStage.PRE_DOWNLOAD)
        if report.errorCode != 0:
            # terminate the deployment.
            return None
        numeric_deploy_stage = DeployStage._NAMES_TO_VALUES(report.get('deployStage')) 
        if report.status == AgentStatus.SUCCEEDED:
            ...
            if numeric_deploy_stage == DeployStage.SERVING_BUILD:
                return {'opCode': OperationCode.NOOP}

            # move to next deploy stage
            next_deploy_stage = _DEPLOY_STAGE_TRANSITIONS.get(numeric_deploy_stage)
            return _new_response_value(next_deploy_stage)

        if report.status == AgentStatus.UNKNOWN:
            # retry current stage
            return _new_response_value(numeric_deploy_stage)

        # terminate deployment
        return None

    def _new_response_value(numeric_deploy_stage):
