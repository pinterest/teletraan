...
_DEPLOY_STAGE_TRANSITIONS = dict(
    [(i, i + 1) for i in range(DeployStage.PRE_DOWNLOAD, DeployStage.SERVING_BUILD)]
)
...
class ServerlessClient(BaseClient):
    ...
    def __init__(
        ...
        self._build: dict[str, str] = json.loads(
            utils.check_not_none(build, "build can not be None")
        )

        self._script_variables: dict[str, str] = json.loads(
        ...
        self._deploy_stage: DeployStage = (
            deploy_stage if deploy_stage is not None else DeployStage.PRE_DOWNLOAD
        )
    ...
    def _create_response(self, report) -> Optional[PingResponse]:
        ...
        if (
            report is None
            or report.deployId is None
            or report.deployId != self._deploy_id
        ):
