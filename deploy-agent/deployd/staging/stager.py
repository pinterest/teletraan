...
class Stager(object):
    ...
    def __init__(self, config, build, target, env_name, transformer=None):
        ...
        self._transformer = transformer or Transformer(agent_dir=agent_dir, env_name=env_name)
        self._build = build
        self._target = target
    ...
    def enable_package(self):
        ...
        try:
            ...
            log.info("{} points to {} (previously {})".format(self._target, self.get_enabled_build(), old_build))
            self.transform_script()
        except Exception:
    ...
    def get_enabled_build(self):
        ...
        if not os.path.exists(self._target):
            if os.path.islink(self._target) and not os.path.lexists(self._target):
                ...
                log.info("{} points to {} which does not exist".format(self._target, symlink_target))
            else:
    ...
    def transform_script(self):
        ...
        self._transformer.transform_scripts(
            script_dir=template_dir, template_dirname=self._template_dirname, script_dirname=self._script_dirname
        )
...
def main():
    ...
    parser.add_argument(
        ...
        dest='config_file',
        default=None,
        help="the deploy agent conf file filename path. If none, " "/etc/deployagent.conf will be used",
    )
    parser.add_argument(
        '-v',
        '--build-id',
    ...
    parser.add_argument('-t', '--target', dest='target', required=True, help="The deploy target directory name.")
    parser.add_argument(
    ...
    result = Stager(config=config, build=args.build, target=args.target, env_name=args.env_name).enable_package()
