...
def create_sc_timing(name, value, sample_rate=1.0, tags=None) -> None:
    if IS_PINTEREST:
        ...
        client = MetricClient()
        client.send(
            mtype=mtype, name=name, value=value, sample_rate=sample_rate, tags=tags
        )
    else:
        return
...
def create_sc_increment(name, sample_rate=1.0, tags=None) -> None:
    if IS_PINTEREST:
        mtype = "increment"
        client = MetricClient()
        client.send(mtype=mtype, name=name, sample_rate=sample_rate, tags=tags)
    else:
        return
...
class Stat:
    ...
    def __init__(
        self, mtype=None, name=None, value=None, sample_rate=None, tags=None, ins=None
    ) -> None:
        self.mtype = mtype
        self.name = name
        self.value = value
...
class MetricClient:
    ...
    @staticmethod
    def _parse_stat(
        mtype=None, name=None, value=None, sample_rate=None, tags=None
    ) -> Stat:
    ...
    def _send_mtype(self) -> None:
        ...
        if self.stat.mtype == "increment":
            ...
            func_v2(
                name_sc_v2,
                self.stat.value,
                sample_rate=self.stat.sample_rate,
                tags=self.stat.tags,
            )
            func(
                self.stat.name,
    ...
    def send_context_timer(self, name, value, sample_rate=None, tags=None) -> None:
        ...
        """
        self.send(
            mtype="timing", name=name, value=value, sample_rate=sample_rate, tags=tags
        )

    def send(
        self, mtype=None, name=None, value=None, sample_rate=None, tags=None
    ) -> None:
        ...
        if self.is_healthy():
            ...
            if self.cache and (self.cache.exists() and not self.cache.is_empty()):
                self._flush_cache()
            # send metric
            self.stat = self._parse_stat(
                mtype=mtype, name=name, value=value, sample_rate=sample_rate, tags=tags
            )
            self._send()
        else:
            # health check failed, write stat to cache
            stat = self._parse_stat(
                mtype=mtype, name=name, value=value, sample_rate=sample_rate, tags=tags
            )
