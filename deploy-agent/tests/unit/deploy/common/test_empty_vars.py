...
class TestScriptVariablesConfig(unittest.TestCase):
    def setUp(self):
        ...
        self.script_config_path = os.path.join(self.temp_dir, f"{self.env_name}_SCRIPT_CONFIG")
        ...
        self.agent = DeployAgent(client=MagicMock(), conf=self.mock_config, executor=self.mock_executor)
