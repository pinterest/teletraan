...
public class Utils {
   ...
   *
   * <p>The meter is built with a single measurement of type VALUE. This provides a workaround that
   * micrometer doesn't support dynamic tags. It's done by removing the existing meter and creating
   * a new one with the same name and tags. Otherwise, the new value will not be reported because
   * the registry only keeps 1 meter and the measurement is immutable after creation.
   *
  ...
  public static Meter registerOrReplaceMeter(
    ...
    return Meter.builder(
            meterName, type, Arrays.asList(new Measurement(() -> value, Statistic.VALUE)))
        .tags(tags)
        .register(registry);
  }
}
