...
public class DBBuildDAOImpl implements BuildDAO {
  ...
  public List<String> getBuildNames(String nameFilter, int pageIndex, int pageSize)
    ...
    if (StringUtils.isNotEmpty(nameFilter)) {
      return run
          .query(GET_BUILD_NAMES, SingleResultSetHandlerFactory.<String>newListObjectHandler(),
              String.format("%%%s%%", nameFilter), (pageIndex - 1) * pageSize, pageSize);
    } else {
      return run
          .query(GET_BUILD_NAMES2, SingleResultSetHandlerFactory.<String>newListObjectHandler(),
  ...
  public List<BuildBean> getAcceptedBuilds(String buildName, String branch, Interval interval,
    ...
    if (StringUtils.isNotEmpty(branch)) {
      ...
      return new QueryRunner(dataSource).query(
          String.format(GET_ACCEPTED_BUILDS_BETWEEN_TEMPLATE, buildName, interval.getStartMillis(),
  ...
  public List<String> getAllBuildNames() throws Exception {
    QueryRunner run = new QueryRunner(this.dataSource);
    return run
        .query(GET_ALL_BUILD_NAMES, SingleResultSetHandlerFactory.<String>newListObjectHandler());
  }
  ...
  public List<BuildBean> getBuildsFromIds(Collection<String> ids) throws Exception {
    ...
    return run
        .query(String.format(GET_LIST_OF_BUILDS_BY_IDs, QueryUtils.genStringGroupClause(ids)), h);
