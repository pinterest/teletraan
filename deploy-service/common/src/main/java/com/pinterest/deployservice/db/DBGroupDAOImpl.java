...
public class DBGroupDAOImpl implements GroupDAO {
    ...
    private static final String GET_HOSTS =
            "SELECT * FROM hosts_and_envs WHERE env_id=?";
    private static final String INSERT_HOST =
            "INSERT INTO hosts_and_envs SET host_name=?,env_id=?";
    ...
    private static final String GET_GROUPS =
            "SELECT * FROM groups_and_envs WHERE env_id=?";
    ...
    public List<String> getEnvsByGroupName(String groupName) throws Exception {
        return new QueryRunner(dataSource).query(GET_ENV_BY_GROUP, SingleResultSetHandlerFactory.<String>newListObjectHandler(), groupName);
    }
