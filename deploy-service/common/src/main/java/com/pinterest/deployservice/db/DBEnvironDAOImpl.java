...
public class DBEnvironDAOImpl implements EnvironDAO {
    private static final String INSERT_ENV_TEMPLATE = "INSERT INTO environs SET %s";
    private static final String UPDATE_ENV_BY_ID_TEMPLATE = "UPDATE environs SET %s WHERE env_id=?";
    private static final String UPDATE_ENV_BY_STAGE_TEMPLATE =
            "UPDATE environs SET %s WHERE env_name=? AND stage_name=?";
    private static final String UPDATE_ALL = "UPDATE environs SET %s";
    private static final String SET_EXTERNAL_ID =
            "UPDATE environs SET external_id=? WHERE env_name=? AND stage_name=?";
    private static final String GET_ENV_BY_ID = "SELECT * FROM environs WHERE env_id=?";
    ...
    private static final String GET_ENV_BY_NAME = "SELECT * FROM environs WHERE env_name=?";
    private static final String GET_ENV_BY_STAGE =
            "SELECT * FROM environs WHERE env_name=? AND stage_name=?";
    private static final String GET_ENV_BY_CLUSTER = "SELECT * FROM environs WHERE cluster_name=?";
    ...
    private static final String DELETE_ENV = "DELETE FROM environs WHERE env_id=?";
    ...
    private static final String GET_ALL_ENV_IDS = "SELECT env_id FROM environs";
    private static final String GET_ALL_ENVS = "SELECT * FROM environs";
    ...
    private static final String GET_ENV_BY_CONSTRAINT_ID =
            "SELECT * FROM environs WHERE deploy_constraint_id = ?";
    ...
    @Override
    public List<String> getAllEnvNames(String nameFilter, long pageIndex, int pageSize)
            throws Exception {
    ...
    @Override
    public List<String> getOverrideHosts(String envId, String envName, String envStage)
            throws Exception {
        return new QueryRunner(dataSource)
                .query(
                        GET_OVERRIDE_HOSTS_BY_CAPACITY,
                        SingleResultSetHandlerFactory.<String>newListObjectHandler(),
                        envId,
                        envName,
                        envStage);
    ...
    @Override
    public List<String> getTotalCapacityHosts(String envId, String envName, String envStage)
            throws Exception {
        List<String> totalHosts =
                new QueryRunner(dataSource)
                        .query(
                                GET_HOSTS_BY_CAPACITY,
                                SingleResultSetHandlerFactory.<String>newListObjectHandler(),
                                envId,
                                envId);
        List<String> overrideHosts = getOverrideHosts(envId, envName, envStage);
    ...
    public List<EnvironBean> getEnvsByHost(String host) throws Exception {
        ...
        List<EnvironBean> hostEnvs =
                new QueryRunner(dataSource).query(GET_ENVS_BY_HOST_TMPL, h, host);
        Set<EnvironBean> envSet =
                new TreeSet<EnvironBean>(Comparator.comparing(EnvironBean::getEnv_id));
    ...
    public List<EnvironBean> getEnvsByGroups(Collection<String> groups) throws Exception {
        ...
        List<EnvironBean> groupEnvs =
                new QueryRunner(dataSource)
                        .query(
                                String.format(
                                        GET_ENVS_BY_GROUPS_TMPL,
                                        QueryUtils.genStringPlaceholderList(groups.size())),
                                h,
                                groups.toArray());
        Set<EnvironBean> envSet =
                new TreeSet<EnvironBean>(Comparator.comparing(EnvironBean::getEnv_id));
