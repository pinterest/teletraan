...
public class DBAgentDAOImpl implements AgentDAO {
    ...
    private static final String DELETE_AGENT = "DELETE FROM agents WHERE host_id=? AND env_id=?";
    private static final String DELETE_AGENT_BY_ID = "DELETE FROM agents WHERE host_id=?";
    private static final String GET_AGENT_BY_HOST = "SELECT * FROM agents WHERE host_name=?";
    private static final String GET_AGENT_BY_HOSTID = "SELECT * FROM agents WHERE host_id=?";
    private static final String GET_ALL_AGENT_BY_ENV = "SELECT * FROM agents WHERE env_id=?";
    private static final String GET_AGENT_BY_ENV_AND_FIRST_DEPLOY_TIME =
            "SELECT * FROM agents WHERE env_id=? AND first_deploy_time>?";
    private static final String GET_BY_IDS = "SELECT * FROM agents WHERE host_id=? AND env_id=?";
    ...
    private static final String COUNT_ALL_AGENT_BY_ENV =
            "SELECT COUNT(*) FROM agents WHERE env_id=?";
    private static final String COUNT_ALL_AGENT_BY_ENV_NAME =
            "SELECT COUNT(*) FROM agents WHERE env_name=?";
    private static final String COUNT_SERVING_TOTAL =
            "SELECT COUNT(*) FROM agents WHERE env_id=? AND deploy_stage=?";
    private static final String COUNT_SERVING_AND_NORMAL_TOTAL =
    ...
    @Override
    public void updateMultiple(Collection<String> hostIds, String envId, AgentBean agentBean)
            throws Exception {
        SetClause setClause = agentBean.genSetClause();
        String clause =
                String.format(
                        UPDATE_AGENTS_BY_HOSTIDS,
                        setClause.getClause(),
                        QueryUtils.genStringPlaceholderList(hostIds.size()));
    ...
    public List<AgentBean> getByEnvAndFirstDeployTime(String envId, long time) throws Exception {
        ...
        return new QueryRunner(dataSource)
                .query(GET_AGENT_BY_ENV_AND_FIRST_DEPLOY_TIME, h, envId, time);
    }
    ...
    public long countNonFirstDeployingAgent(String envId) throws Exception {
        Long n =
                new QueryRunner(dataSource)
                        .query(
                                GET_NON_FIRST_TIME_DEPLOY_TOTAL,
                                SingleResultSetHandlerFactory.<Long>newObjectHandler(),
                                envId);
        return n == null ? 0 : n;
    }
    ...
    public long countServingAndNormalTotal(String envId) throws Exception {
        Long n =
                new QueryRunner(dataSource)
                        .query(
                                COUNT_SERVING_AND_NORMAL_TOTAL,
                                SingleResultSetHandlerFactory.<Long>newObjectHandler(),
                                envId,
                                DeployStage.SERVING_BUILD.toString(),
    ...
    public long countFinishedAgentsByDeploy(String deployId) throws Exception {
        Long n =
                new QueryRunner(dataSource)
                        .query(
                                COUNT_FINISHED_AGENTS_BY_DEPLOY,
                                SingleResultSetHandlerFactory.<Long>newObjectHandler(),
                                deployId);
        return n == null ? 0 : n;
    }
    ...
    @Override
    public long countDeployingAgentWithHostTag(String envId, String tagName, String tagValue)
            throws Exception {
        Long n =
                new QueryRunner(dataSource)
                        .query(
                                GET_DEPLOYING_TOTAL_WITH_HOST_TAG,
                                SingleResultSetHandlerFactory.<Long>newObjectHandler(),
                                envId,
                                envId,
                                DeployStage.SERVING_BUILD.toString(),
    ...
    @Override
    public long countFinishedAgentsByDeployWithHostTags(
            String envId, String deployId, String tagName, List<String> tagValues)
            throws Exception {
        List<Object> params =
        ...
        Long n =
                new QueryRunner(dataSource)
                        .query(
                                String.format(
                                        COUNT_FINISHED_AGENTS_BY_DEPLOY_WITH_HOST_TAGS,
