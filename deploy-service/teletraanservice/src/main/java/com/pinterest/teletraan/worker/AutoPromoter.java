...
public class AutoPromoter implements Runnable {
  ...
  public long getScheduleStartTime(EnvironBean currEnvBean, DeployBean currDeployBean,
    ...
    if (currDeployBean != null) {
      ...
      BuildBean buildBean = getBuildCandidate(currEnvBean, new Interval(0, Long.MAX_VALUE), 1);
      if (buildBean != null) {
  ...
  public PromoteResult computePromoteBuildResult(EnvironBean currEnvBean, DeployBean currDeployBean,
    ...
    if (!StringUtils.isEmpty(schedule)) {
      ...
      if (autoDeployDueDate.getTime() != 0) {
        end_time = autoDeployDueDate.getTime();
      }
    ...
    if (currentBuildDate < end_time) {
      BuildBean
          buildBean =
          getBuildCandidate(currEnvBean, new Interval(currentBuildDate, end_time), size);
      if (buildBean != null) {
        return new PromoteResult()
  ...
  void handleFailedPromote(PromoteBean promoteBean, EnvironBean currEnvBean, String deployId)
    ...
    if (autoPromotePolicy == PromoteFailPolicy.ROLLBACK) {
      LOG.info(
          "Rollback deploy {} and disable auto deploy for env {}/{} since deploy failed and env "
  ...
  boolean autoDeployDue(DeployBean deployBean, String cronExpressionString) {
    Date date = new Date();
    return !date.before(getScheduledCheckDueTime(deployBean.getStart_date(), cronExpressionString));
  }
  ...
  public Date getScheduledCheckDueTime(long start_date, String cronExpressionString) {
    ...
    try {
      if (!CronExpression.isValidExpression(cronExpressionString)) {
        LOG.error(
            String.format("Cron expression %s is not valid. Ignore it.", cronExpressionString));
        return ret;
      }
  ...
  BuildBean getBuildCandidate(EnvironBean envBean, Interval interval, int size) throws Exception {
    ...
    List<BuildBean> buildBeans = buildDAO.getAcceptedBuilds(buildName, scmBranch, interval, size);
    ...
    if (connection != null) {
      ...
      try {
        ...
        if (predDeployBean != null) {
          ...
          String
              newDeployId =
              deployHandler.promote(currEnvBean, predDeployBean.getDeploy_id(), description,
                  AUTO_PROMOTER_NAME);
