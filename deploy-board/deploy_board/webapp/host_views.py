    ...
    for agent in agents:
        ...
        if agent_env and agent.get("lastErrno", 0) != 0:
            agent_wrapper["error"] = agents_helper.get_agent_error(
                request, agent_env["envName"], agent_env["stageName"], hostname
            )
        if agent["state"] == "UNREACHABLE":
            is_unreachable = True
        if (
            agent_env
            and agent_env["systemPriority"]
            and agent_env["systemPriority"] > 0
        ):
            agent_wrappers["sidecars"].append(agent_wrapper)
        else:
            agent_wrappers["services"].append(agent_wrapper)

    agent_wrappers["sidecars"] = sorted(
        agent_wrappers["sidecars"], key=lambda x: x["agent"]["lastUpdateDate"]
    )
    agent_wrappers["services"] = sorted(
        agent_wrappers["services"], key=lambda x: x["agent"]["lastUpdateDate"]
    )
        ...
        for host in hosts:
            if host and host.get("groupName"):
                group_info = autoscaling_groups_helper.get_group_info(
                    request, host.get("groupName")
                )
                if (
                    group_info
                    and group_info.get("launchInfo")
                    and group_info.get("launchInfo")["asgStatus"] == "ENABLED"
                ):
...
def get_account_id(hosts):
    for host in hosts:
        if (
            host
            and host.get("accountId")
            and host.get("accountId") not in {"NULL", "null"}
        ):
...
def get_host_details(host_id, account_id):
    ...
    launch_time = datetime.fromtimestamp(
        launch_time / 1000, pytz.timezone("America/Los_Angeles")
    ...
    if IS_PINTEREST and PHOBOS_URL:
        ...
        if host_ip is not None:
            phobos_link = PHOBOS_URL + (
                host_name if account_id == AWS_PRIMARY_ACCOUNT else host_ip
            )
...
class HostDetailView(View):
    def get(self, request, name, stage, hostname):
        ...
        for stage_name in stages:
            if stage_name != stage:
                hosts = environs_helper.get_env_capacity(
                    request, name, stage_name, capacity_type="HOST"
                )
        ...
        has_failed_sidecars_or_host_env_sidecars = any(
            wrapper
            for wrapper in agent_wrappers["sidecars"]
            if (
                wrapper["env"]
                and wrapper["env"]["envName"]
                and wrapper["env"]["stageName"]
                and wrapper["env"]["envName"] == name
                and wrapper["env"]["stageName"] == stage
            )
            or is_agent_failed(wrapper["agent"])
        )

        host_details = get_host_details(host_id, account_id)

        termination_limit = environs_helper.get_env_by_stage(request, name, stage).get(
            "terminationLimit"
        )
