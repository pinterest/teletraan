...
def get_accounts(report):
    ...
    for agentStat in report.agentStats:
        if "accountId" in agentStat.agent and is_valid_account_id(
        ...
        if cluster:
            ...
            if not id:
                account = accounts_helper.get_default_account(
                    request, cell, provider=provider
                )
            else:
                account = accounts_helper.get_by_cell_and_id(
