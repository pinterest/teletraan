...
class BaseClient(object):
    def __init__(
        self, url_prefix, version, proxy_http=None, proxy_https=None, bearer=False
    ):
        ...
        def api(path, token=None, params=None, data=None):
            ...
            if token:
                headers["Authorization"] = (
                    "bearer %s" % token if self.bearer else "token %s" % token
                )
            ...
            if response.status_code >= 400 and response.status_code < 600:
                try:
                    if "access_token=" in response.text:
                        bad_text = (
                            response.text.split("access_token=")[1]
                            .split('"')[0]
                            .replace("\\", "")
                        )
            ...
            if response.status_code == 403:
                raise NotAuthorizedException(
                    f"{UNAUTHORIZED_ERROR_TEXT}. Server message: {response.json()['message']}"
                )
