...
def get_hosts(request, env_name, stage_name):
    return deploy_client.get("/envs/%s/%s/hosts" % (env_name, stage_name), request.teletraan_user_id.token)
...
def get_host_tags(request, env_name, stage_name, tag_name):
    return deploy_client.get("/envs/%s/%s/host_tags/%s" % (env_name, stage_name, tag_name),
                             request.teletraan_user_id.token)


def remove_host_tags(request, env_name, stage_name, tag_name):
    return deploy_client.delete("/envs/%s/%s/host_tags/%s" % (env_name, stage_name, tag_name),
                                request.teletraan_user_id.token)


def get_ec2_host_tags(request, env_name, stage_name):
    return deploy_client.get("/envs/%s/%s/host_tags?ec2Tags=true" % (env_name, stage_name),
                             request.teletraan_user_id.token)


def create_deploy_constraint(request, env_name, stage_name, data):
    return deploy_client.post("/envs/%s/%s/deploy_constraint" % (env_name, stage_name), request.teletraan_user_id.token,
                              data=data)


def remove_deploy_constraint(request, env_name, stage_name):
    return deploy_client.delete("/envs/%s/%s/deploy_constraint" % (env_name, stage_name), request.teletraan_user_id.token)
...
def get_deploy_constraint(request, env_name, stage_name):
    return deploy_client.get("/envs/%s/%s/deploy_constraint" % (env_name, stage_name), request.teletraan_user_id.token)
