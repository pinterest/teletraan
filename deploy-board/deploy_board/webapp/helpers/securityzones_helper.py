...
def create_security_zone(request, security_zone_info):
    return rodimus_client.post(
        "/security_zones", request.teletraan_user_id.token, data=security_zone_info
    )
...
def get_all(request, index, size):
    ...
    return rodimus_client.get(
        "/security_zones", request.teletraan_user_id.token, params=params
    )
...
def get_by_provider_and_cell_name(request, account_id, provider, cell_name):
    ...
    if cell_name:
        return rodimus_client.get(
            f"/security_zones/cell/{cell_name}{query}", request.teletraan_user_id.token
        )
    return rodimus_client.get(
        f"/security_zones/provider/{provider}{query}", request.teletraan_user_id.token
    )


def get_by_id(request, security_zone_id):
    return rodimus_client.get(
        "/security_zones/%s" % security_zone_id, request.teletraan_user_id.token
    )
