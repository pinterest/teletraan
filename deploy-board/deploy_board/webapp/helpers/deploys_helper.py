...
def deploy(request, env_name, stage_name, build_id, description=None):
    ...
    return deployclient.post("/envs/%s/%s/deploys" % (env_name, stage_name),
                             request.teletraan_user_id.token, params=params)


def pause(request, env_name, stage_name):
    params = [("actionType", "PAUSE")]
    return deployclient.post("/envs/%s/%s/deploys/current/actions" % (env_name, stage_name),
                             request.teletraan_user_id.token, params=params)
...
def get_current(request, env_name, stage_name):
    return deployclient.get("/envs/%s/%s/deploys/current" % (env_name, stage_name),
                            request.teletraan_user_id.token)


def update_progress(request, env_name, stage_name):
    return deployclient.put("/envs/%s/%s/deploys/current/progress" % (env_name, stage_name),
                            request.teletraan_user_id.token)
