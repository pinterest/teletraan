...
def create_placement(request, placement_info):
    return rodimus_client.post(
        "/placements", request.teletraan_user_id.token, data=placement_info
    )
...
def get_all(request, index, size):
    ...
    return rodimus_client.get(
        "/placements", request.teletraan_user_id.token, params=params
    )
...
def get_by_provider_and_cell_name(request, account_id, provider, cell_name):
    ...
    if cell_name:
        return rodimus_client.get(
            f"/placements/cell/{cell_name}{query}", request.teletraan_user_id.token
        )
    return rodimus_client.get(
        f"/placements/provider/{provider}{query}", request.teletraan_user_id.token
    )


def get_by_id(request, placement_id):
    return rodimus_client.get(
        "/placements/%s" % placement_id, request.teletraan_user_id.token
    )
    ...
    return [
        {
            k: placement[k]
            for k in placement
            if k in ["id", "capacity", "abstract_name", "provider_name"]
        }
        for placement in all_placements
        if placement["id"] in current_placement_ids
    ]
