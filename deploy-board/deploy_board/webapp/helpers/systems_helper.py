...
def send_chat_message(request, from_name, to_name, message):
    ...
    return deployclient.post("/system/send_chat_message", request.teletraan_user_id.token,
                             data=data)


def ping(request, ping_request):
    return deployclient.post("/system/ping", request.teletraan_user_id.token, data=ping_request)
