...
from deploy_board.settings import (
    IS_PINTEREST,
    RODIMUS_CLUSTER_REPLACEMENT_WIKI_URL,
    RODIMUS_AUTO_CLUSTER_REFRESH_WIKI_URL,
...
if IS_PINTEREST:
    from deploy_board.settings import (
        DEFAULT_PROVIDER,
        DEFAULT_CMP_IMAGE,
        DEFAULT_CMP_ARM_IMAGE,
        DEFAULT_CMP_HOST_TYPE,
        DEFAULT_CMP_ARM_HOST_TYPE,
        DEFAULT_CMP_PINFO_ENVIRON,
        DEFAULT_CMP_ACCESS_ROLE,
        DEFAULT_CELL,
        DEFAULT_ARCH,
        DEFAULT_PLACEMENT,
...
class EnvCapacityBasicCreateView(View):
    ...
    def post(self, request, name, stage):
        ...
        try:
            ...
            if "configs" in cluster_info:
                for field in TELETRAAN_CLUSTER_READONLY_FIELDS:
                    if field in cluster_info["configs"]:
                        msg = "Teletraan does not support user to change %s %s" % (
                            field,
                            cluster_info[field],
                        )
                        log.error(msg)
            ...
            cluster_info["statefulStatus"] = (
                clusters_helper.StatefulStatuses.get_status(
                    cluster_info["statefulStatus"]
                )
            )
            clusters_helper.create_cluster_with_env(
                request, cluster_name, name, stage, cluster_info
            )
            ...
            if ret == 200:
                ...
                return HttpResponse(
                    exception, status=ret, content_type="application/json"
                )
...
class EnvCapacityAdvCreateView(View):
    def get(self, request, name, stage):
        ...
        capacity_creation_info = {
            ...
            "defaultAccountId": default_account["id"]
...
class ClusterConfigurationView(View):
    def get(self, request, name, stage):
        ...
        asg_cluster = autoscaling_groups_helper.get_group_info(
            request, current_cluster["clusterName"]
        )
        current_cluster["asg_info"] = asg_cluster
        is_managed_resource = current_cluster["isManagedResource"]
        ...
        return render(
            request,
            "clusters/cluster_configuration.html",
            {
                "env": env,
                "capacity_creation_info": json.dumps(capacity_creation_info),
                "default_cmp_image": DEFAULT_CMP_IMAGE,
    ...
    def post(self, request, name, stage):
        try:
            ...
            cluster_info = json.loads(request.body)
            log.info("YaqinDebug Update Cluster Configuration with {}", cluster_info)
            log.info("YaqinDebug: {}", "abcdefg")
            log.info("YaqinDebug: {}", cluster_info["bandwidthWeighting"])

            current_cluster = get_current_cluster(request, name, stage, env=env)
            log.info("lalala getting current Cluster Configuration is {}", current_cluster)
            if "configs" in current_cluster and "configs" in cluster_info:
                for field in TELETRAAN_CLUSTER_READONLY_FIELDS:
                    if (
                        field in current_cluster["configs"]
                        and field in cluster_info["configs"]
                    ):
                        if (
                            current_cluster["configs"][field]
                            != cluster_info["configs"][field]
                        ):
                            log.error(
                                "Teletraan does not support user to update %s %s"
                                % (field, cluster_info["spiffe_id"])
                            )
                            raise TeletraanException(
                                "Teletraan does not support user to update %s" % field
                            )

                    if (
                        field in current_cluster["configs"]
                        and field not in cluster_info["configs"]
                    ):
                        log.error(
                            "Teletraan does not support user to remove %s %s"
                            % (field, cluster_info[field])
                        )
                        raise TeletraanException(
                            "Teletraan does not support user to remove %s" % field
                        )
            cluster_info["statefulStatus"] = (
                clusters_helper.StatefulStatuses.get_status(
                    cluster_info["statefulStatus"]
...
def get_base_image_events(request, image_id):
    ...
    update_events = sorted(
        update_events, key=lambda event: event["create_time"], reverse=True
    )
    ...
    latest_update_events = baseimages_helper.get_latest_image_update_events(
        update_events
    )
    progress_info = baseimages_helper.get_base_image_update_progress(
        latest_update_events
    )
    cluster_statuses = [
        {"cluster_name": event["cluster_name"], "status": event["status"]}
        for event in latest_update_events
    ]
    cluster_statuses = sorted(
        cluster_statuses, key=lambda event: event["status"], reverse=True
    )

    return render(
        request,
        "clusters/base_images_events.html",
        {
...
def get_image_names_by_provider_and_cell_and_arch(request, provider, cell, arch):
    image_names = baseimages_helper.get_image_names_by_arch(
        request, provider, cell, arch
    )
    return HttpResponse(json.dumps(image_names), content_type="application/json")
...
def get_placements_by_provider_and_cell(request, provider, cell):
    ...
    data = placements_helper.get_by_provider_and_cell_name(
        request, account_id, provider, cell
    )
...
def create_ui_accounts(accounts):
    ...
    for account_with_cell in id_to_account_with_cells.values():
        res.append(
            create_ui_account(account_with_cell["account"], account_with_cell["cells"])
        )
...
def get_host_types_mapping(request):
    ...
    return render(
        request,
        "clusters/host_types_mapping.html",
        {
...
def get_host_type_mapping_by_id(request, host_type_id):
    ...
    host_types = hosttypes_helper.get_by_provider(request, "AWS")
    contents = render_to_string(
        "clusters/modify_host_type_mapping_modal.tmpl",
        {
...
def create_host_type_mapping(request):
    ...
    if params["thirdHostType"] == "None":
        ...
        host_type_mapping_info["backupIds"] = [
            params["secondHostType"],
            params["thirdHostType"],
        ]
...
def modify_host_type_mapping(request):
    try:
        ...
        if host_type_mapping_info["thirdHostType"] == "None":
            ...
            updated_info["backupIds"] = [
                host_type_mapping_info["secondHostType"],
                host_type_mapping_info["thirdHostType"],
            ]
        ...
        hosttypesmapping_helper.modify_host_type_mapping(
            request, host_type_id, updated_info
        )
    except NotAuthorizedException as e:
        log.error("Have an NotAuthorizedException error {}".format(e))
        return HttpResponse(e, status=403, content_type="application/json")
        ...
        return HttpResponse(e, status=500, content_type="application/json")
    return HttpResponse(
        json.dumps(host_type_mapping_info), content_type="application/json"
    )
...
def get_host_type_by_id(request, host_type_id):
    ...
    contents = render_to_string(
        "clusters/modify_host_type_modal.tmpl",
        {
            "arches_list": arches_list,
            "provider_list": provider_list,
            "host_type": host_type,
...
def get_security_zones_by_provider(request):
    ...
    contents = render_to_string(
        "clusters/get_security_zone.tmpl",
        {
            "security_zones": security_zones,
...
def clone_cluster(request, src_name, src_stage):
    ...
    try:
        ...
        environs_helper.update_env_basic_config(
            request, dest_name, dest_stage, data={"clusterName": dest_cluster_name}
        )
        ...
        return HttpResponse(
            json.dumps(src_cluster_info), content_type="application/json"
        )
    except NotAuthorizedException as e:
        log.error("Have an NotAuthorizedException error {}".format(e))
        ...
        log.error(
            "Failed to clone cluster env_name: %s, stage_name: %s"
            % (src_name, src_stage)
        )
...
def terminate_hosts(request, name, stage):
    ...
    environ_hosts_helper.stop_service_on_host(
        request, name, stage, host_ids, replace_host
    )
    return redirect("/env/{}/{}/".format(name, stage))
...
def gen_cluster_replacement_view(request, name, stage):
    ...
    get_cluster_replacement_body = {"clusterName": cluster_name}
    ...
    content = render_to_string(
        "clusters/cluster-replacements.tmpl",
        {
...
def gen_auto_cluster_refresh_view(request, name, stage):
    ...
    try:
        ...
        messages.warning(
            request, "failed to retrieve group info", "cluster-replacements"
        )
    ...
    try:
        if auto_refresh_config is None:
            auto_refresh_config = (
                clusters_helper.get_default_cluster_auto_refresh_config(
                    request, cluster_name
                )
            )
...
def get_cluster_replacement_details(request, name, stage, replacement_id):
    ...
    content = render_to_string(
        "clusters/cluster-replacement-details.tmpl",
        {
            "env": env,
            "env_name": name,
            "env_stage": stage,
...
def start_cluster_replacement(request, name, stage):
    ...
    try:
        clusters_helper.start_cluster_replacement(
            request, data=start_cluster_replacement
        )
        messages.success(
            request, "Cluster replacement started successfully.", "cluster-replacements"
        )
    except TeletraanException as ex:
        if "already in progress" in str(ex) and "409" in str(ex):
            messages.warning(
                request,
                "Cluster replacement is already in progress.",
...
def submit_auto_refresh_config(request, name, stage):
    ...
    try:
        clusters_helper.submit_cluster_auto_refresh_config(
            request, data=auto_refresh_config
        )
        cluster = clusters_helper.get_cluster(request, cluster_name)
        ...
        if group_info:
            ...
            autoscaling_groups_helper.update_group_info(
                request, cluster_name, group_info["groupInfo"]
            )
        messages.success(
            request, "Auto refresh config saved successfully.", "cluster-replacements"
        )
    except IllegalArgumentException:
...
def gen_replacement_config(request):
    ...
    if params["checkpointPercentages"]:
        checkpointPercentages = [
            int(x) for x in params["checkpointPercentages"].split(",")
        ]
    ...
    if params["availabilitySettingRadio"] == "launchBeforeTerminate":
        ...
        if (
            params["maxHealthyPercentage"] is None
            or len(params["maxHealthyPercentage"]) == 0
        ):
...
def perform_cluster_replacement_action(
    request, name, stage, action, includeMessage=True
):
    cluster_name = common.get_cluster_name(request, name, stage)
    log.info(
        "Starting to {0} cluster replacement for cluster {1}".format(
            action, cluster_name
        )
    )

    try:
        clusters_helper.perform_cluster_replacement_action(
            request, cluster_name, action
        )
        if includeMessage:
            if action == "pause":
                ...
                messages.success(
                    request, "Resumed successfully", "cluster-replacements"
                )
            else:
                messages.success(
                    request, "Canceled successfully", "cluster-replacements"
                )
    except TeletraanException as ex:
        if "active replacement" in str(ex) and "409" in str(ex):
            messages.warning(
                request,
                "There is no active replacement to cancel",
                "cluster-replacements",
            )
        else:
            messages.warning(request, str(ex), "cluster-replacements")
    ...
    if state == "COMPLETED":
        if status == "SUCCEEDED":
            ...
            return {
                ...
                "successRate": "{}% ({}/{})".format(
                    progress_rate, succeeded, succeeded
                ),
                "description": msg,
            }
        else:
            ...
            return {
                ...
                "successRatePercentage": progress_rate,
                "successRate": "{}% ({}/{})".format(
                    progress_rate, succeeded, current_capacity
                ),
                "description": msg,
            }
...
class ClusterBaseImageHistoryView(View):
    def get(self, request, name, stage):
        ...
        base_images_update_events = (
            baseimages_helper.get_image_update_events_by_cluster(request, cluster_name)
        )
