...
from deploy_board.settings import (
    ...
    TELETRAAN_CODE_FREEZE_URL,
    TELETRAAN_JIRA_SOURCE_URL,
    TELETRAAN_TRANSFER_OWNERSHIP_URL,
    TELETRAAN_RESOURCE_OWNERSHIP_WIKI_URL,
    HOST_TYPE_ROADMAP_LINK,
    STAGE_TYPE_INFO_LINK,
)
...
from .accounts import (
    get_accounts,
    get_accounts_from_deploy,
    create_legacy_ui_account,
    add_account_from_cluster,
    add_legacy_accounts,
)
import random
import json
...
def logging_status(request, name, stage):
    ...
    account = _fetch_param_with_cookie(request, "account", ACCOUNT_COOKIE_NAME, "all")
...
def check_logging_status(request, name, stage):
    ...
    kafkaLoggingAddOn = service_add_ons.getKafkaLoggingAddOn(
        serviceName=name, report=report, configStr=configStr
    )
    report.showMode = showMode
    report.sortByStatus = sortByStatus
    html = render_to_string(
        "deploys/deploy_logging_check_update.tmpl",
...
def update_deploy_progress(request, name, stage):
    ...
    if account == AWS_SUB_ACCOUNT:
        for agentStat in report.agentStats:
            if (
                agentStat.agent["accountId"]
                and agentStat.agent["accountId"] == AWS_SUB_ACCOUNT
            ):
        ...
        for agentStat in report.agentStats:
            if (
                not agentStat.agent["accountId"]
                or agentStat.agent["accountId"] == AWS_PRIMARY_ACCOUNT
                or agentStat.agent["accountId"] == "null"
            ):
        ...
        for agentStat in report.agentStats:
            if (
                agentStat.agent["accountId"]
                and agentStat.agent["accountId"] != AWS_PRIMARY_ACCOUNT
                and agentStat.agent["accountId"] != AWS_SUB_ACCOUNT
                and agentStat.agent["accountId"] != "null"
            ):
...
def update_service_add_ons(request, name, stage):
    ...
    serviceName = name
    rateLimitingAddOn = service_add_ons.getRatelimitingAddOn(
        serviceName=serviceName, report=report
    )
    dashboardAddOn = service_add_ons.getDashboardAddOn(
    ...
    html = render_to_string(
        "deploys/deploy_add_ons.tmpl",
        {"serviceAddOns": serviceAddOns, "pinterest": IS_PINTEREST},
    )
...
def check_feedback_eligible(request, username):
    ...
    try:
        if (
            username
            and ratings_helper.is_user_eligible(request, username)
            and IS_PINTEREST
        ):
...
class EnvLandingView(View):
    def get(self, request, name, stage=None):
        ...
        if (
            stage_with_external_id is not None
            and stage_with_external_id["externalId"] is not None
        ):
            try:
                ...
                log.error(
                    "Handling TeletraanException when trying to access nimbus API, error message {}".format(
                        detail
                    )
                )
        ...
        if existing_stage_identifier:
            ...
            try:
                project_info["project_url"] = (
                    environs_helper.get_nimbus_project_console_url(project_name)
                )
            except TeletraanException as detail:
                log.error(
                    "Handling TeletraanException when trying to access nimbus API, error message {}".format(
        ...
        accounts = []
        if IS_PINTEREST:
            pindeploy_config = environs_helper.get_env_pindeploy(
                request, env["envName"], env["stageName"]
            )
            ...
            if capacity_info["cluster"]:
                ...
                remaining_capacity = functools.reduce(
                    lambda s, e: s + e["capacity"], placements, 0
                )
                host_type = hosttypes_helper.get_by_id(
                    request, basic_cluster_info["hostType"]
                )
                host_type_blessed_status = host_type["blessed_status"]
                if (
                    host_type_blessed_status == "DECOMMISSIONING"
                    or host_type["retired"] is True
                ):
                    messages.add_message(
                        request,
                        messages.ERROR,
                        "This environment is currently using a cluster with an unblessed Instance Type. Please refer to "
                        + HOST_TYPE_ROADMAP_LINK
                        + " for the recommended Instance Type",
                    )
        last_cluster_refresh_status = _get_last_cluster_refresh_status(request, env)
        latest_succeeded_base_image_update_event = (
            baseimages_helper.get_latest_succeeded_image_update_event_by_cluster(
                request, env.get("clusterName")
            )
        )
        is_auto_refresh_enabled = _is_cluster_auto_refresh_enabled(request, env)

        cluster_refresh_suggestion_for_golden_ami = _gen_message_for_refreshing_cluster(
            request,
            last_cluster_refresh_status,
            latest_succeeded_base_image_update_event,
            env,
        )
        ...
        if not env["deployId"]:
            ...
            provisioning_hosts = deduplicate_hosts(
                environ_hosts_helper.get_hosts(request, name, stage)
            )
            ...
            report = agent_report.gen_report(
                request, env, progress, sortByStatus=sortByStatus
            )
            report.showMode = showMode
            report.account = account
            report.sortByStatus = sortByStatus
            ...
            if account == AWS_SUB_ACCOUNT:
                ...
                for agentStat in report.agentStats:
                    if (
                        ...
                        report.currentDeployStat.deploy["primaryAcctTotalHostNum"] += 1
                        if agentStat.agent["status"] == "SUCCEEDED":
                            report.currentDeployStat.deploy[
                                "primaryAcctSucHostNum"
                            ] += 1
                        else:
                            report.currentDeployStat.deploy[
                                "primaryAcctFailHostNum"
                            ] += 1
            elif account == "others":
                for agentStat in report.agentStats:
                    if (
...
def _get_asg_suspended_processes(request, env):
    try:
        cluster_name = get_cluster_name(
            request, env.get("envName"), env.get("stageName"), env=env
        )
...
def _gen_message_for_refreshing_cluster(
    request, last_cluster_refresh_status, latest_succeeded_base_image_update_event, env
):
    try:
        ...
        aws_owner_id = accounts_helper.get_aws_owner_id_for_cluster(
        ...
        current_AMI = baseimages_helper.get_by_id(
            request, cluster_config["baseImageId"]
        )
        current_AMI = current_AMI["provider_name"]

        any_host_with_outdated_ami = len(amis) > 1 or (
            len(amis) == 1 and amis[0] != current_AMI
        )

        if (
            any_host_with_outdated_ami
            and latest_succeeded_base_image_update_event is not None
        ):
            if (
                last_cluster_refresh_status is None
                or last_cluster_refresh_status["startTime"] is None
                or last_cluster_refresh_status["startTime"]
                <= latest_succeeded_base_image_update_event["finish_time"]
            ):
                return "The cluster was updated with a new AMI at {} PST and should be replaced to ensure the AMI is applied to all existing hosts.".format(
...
def _gen_deploy_summary(request, deploys, for_env=None):
    ...
    for deploy in deploys:
            ...
            if cluster:
                ...
                if account_key in accounts:
                    ...
                    account = accounts_helper.get_by_cell_and_id(
                        request, cell, id, provider
                    )
                    if account is None:
                        account = accounts_helper.get_default_account(
                            request, cell, provider
                        )
        ...
        if account is None and env and deploy and build_with_tag:
            ...
            progress = deploys_helper.update_progress(
                request, env["envName"], env["stageName"]
            )
            report = agent_report.gen_report(
                request, env, progress, deploy=deploy, build_info=build_with_tag
            )
            deploy_accounts = [
                create_legacy_ui_account(account) for account in get_accounts(report)
            ]
        elif account:
...
def get_all_deploys(request):
    ...
    filter, filter_title, query_string = _gen_deploy_query_filter(
        request,
        from_date,
        from_time,
        to_date,
...
def get_env_deploys(request, name, stage):
    ...
    result = deploys_helper.get_all(
        request, envId=[env["id"]], pageIndex=1, pageSize=DEFAULT_ROLLBACK_DEPLOY_NUM
    )
...
def search_envs(request, filter):
    max_size = 10000
    names = environs_helper.get_all_env_names(
        request, name_filter=filter, index=1, size=max_size
    )
...
def remove_stage(request, name, stage):
    ...
    if (
        current_env_stage_with_external_id is not None
        and current_env_stage_with_external_id["externalId"] is not None
    ):
        try:
            environs_helper.delete_nimbus_identifier(
                request, current_env_stage_with_external_id["externalId"]
            )
        except TeletraanException as detail:
            message = "Failed to delete Nimbus identifier {}. Please verify that identifier no longer exists, Error Message: {}".format(
                current_env_stage_with_external_id["externalId"], detail
            )
            log.error(message)
            messages.add_message(request, messages.ERROR, message)
...
def get_pipeline_url_from_build_info(build):
    if build["publishInfo"] and re.findall(
        "https://[\w\d\-\.]*/job/[\w\d\-\.]*/[\d]*(/)?", build["publishInfo"]
    ):
        return re.sub("/[\d]*(/)?$", "", build["publishInfo"])
    if build["publishInfo"] and re.findall(
        "https://[\w\d\-\.]*/job/[\w\d\-\.]*/?", build["publishInfo"]
    ):
...
def get_builds(request, name, stage):
    ...
    show_lock = False
    if (
        env_promote["type"] == "AUTO"
        and env_promote["predStage"]
        and env_promote["predStage"] == environs_helper.BUILD_STAGE
    ):
        show_lock = True
    ...
    builds = builds_helper.get_builds_and_tags(
        request, name=env["buildName"], pageIndex=index, pageSize=size
    )
    new_builds = []
    for build in builds:
...
def deploy_commit(request, name, stage, commit):
    env = environs_helper.get_env_by_stage(request, name, stage)
    builds = builds_helper.get_builds_and_tags(
        request, name=env.get("buildName", ""), commit=commit
    )
    current_build = None
    scmType = ""
    if env.get("deployId"):
...
def get_deploy(request, name, stage, deploy_id):
    ...
    return render(
        request,
        "environs/env_deploy_details.html",
        {
            "deploy": deploy,
            "csrf_token": get_token(request),
...
def get_hosts_by_deploy(request, name, stage, deploy_id):
    ...
    stages, env = common.get_all_stages(envs, stage)
    progress = deploys_helper.update_progress(request, name, stage)
    agents_wrapper = agent_report.gen_agent_by_deploy(
        progress, deploy_id, ALIVE_STAGE_HOST_REPORT, hostStage
    )
...
def get_unknown_hosts(request, name, stage):
    ...
    progress = deploys_helper.update_progress(request, name, stage)
    agents_wrapper = agent_report.gen_agent_by_deploy(
        progress, env["deployId"], UNKNOWN_HOST_REPORT
    )
    title = "Unknow hosts"
...
def get_provisioning_hosts(request, name, stage):
    ...
    progress = deploys_helper.update_progress(request, name, stage)
    agents_wrapper = agent_report.gen_agent_by_deploy(
        progress, env["deployId"], PROVISION_HOST_REPORT
    )
...
def get_all_hosts(request, name, stage):
    ...
    progress = deploys_helper.update_progress(request, name, stage)
    agents_wrapper = agent_report.gen_agent_by_deploy(
        progress, env["deployId"], TOTAL_HOST_REPORT
    )
    title = "All hosts"
...
def get_failed_hosts(request, name, stage):
    ...
    progress = deploys_helper.update_progress(request, name, stage)
    agents_wrapper = agent_report.gen_agent_by_deploy(
        progress, env["deployId"], FAILED_HOST_REPORT
    )
...
def get_sub_account_hosts(request, name, stage):
    ...
    for agent in agents:
        if (
            not accountIdMap.get(agent["hostId"])
            or accountIdMap.get(agent["hostId"]) == "null"
            or accountIdMap.get(agent["hostId"]) == AWS_PRIMARY_ACCOUNT
        ):
            continue
        if agent["deployId"] not in agents_wrapper:
            agents_wrapper[agent["deployId"]] = []
...
def get_pred_deploys(request, name, stage):
    ...
    if predStage and predStage != "BUILD":
        ...
        if pred_env is not None:
            result = deploys_helper.get_all(
                request, envId=[pred_env["id"]], pageIndex=index, pageSize=size
            )
...
def show_config_comparison(request, name, stage):
    ...
    return render(
        request,
        "configs/env_config_comparison_result.html",
        {
            "envName": name,
            "stageName": stage,
...
def _get_endSha(end_build):
    ...
    if endShaBranch.startswith("hotfix_"):
        ...
        if len(endShaBranchSplits) >= 3:
            endSha = endShaBranchSplits[
                len(endShaBranchSplits) - 1
            ]  # only get the last split
