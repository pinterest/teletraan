...
def search_groups(request, group_name):
    index = int(request.GET.get("page_index", "1"))
    size = int(request.GET.get("page_size", DEFAULT_PAGE_SIZE))
    group_names = autoscaling_groups_helper.get_env_group_names(
        request, index, size, name_filter=group_name
    )
...
def get_launch_config(request, group_name):
    try:
        ...
        appNames = baseimages_helper.get_image_names(
            request, "AWS", settings.DEFAULT_CELL
        )
        appNames = sorted(appNames)
        curr_image = baseimages_helper.get_by_provider_name(
            request, launch_config["imageId"]
        )
        html = render_to_string(
            "groups/launch_config.tmpl",
            {
...
def get_group_config_internal(group_config):
    if group_config:
        ...
        if group_config.get("healthcheckPeriod"):
            group_config["healthcheckPeriod"] = (
                group_config.get("healthcheckPeriod") // 60
            )
        if group_config.get("lifecycleTimeout"):
            group_config["lifecycleTimeout"] = (
                group_config.get("lifecycleTimeout") // 60
            )
        return group_config
    else:
        group_config = {}
    ...
    try:
        ...
        if desired_spiffe_id != actual_spiffe_id:
            ...
            raise TeletraanException(
                "Teletraan does not allow spiffe_id changes from user"
            )

        autoscaling_groups_helper.update_launch_config(
            request, group_name, launchRequest
        )
...
def create_launch_config(request, group_name):
    try:
        ...
        if params["assignPublicIP"] == "True":
            ...
            launchRequest["assignPublicIp"] = False
        autoscaling_groups_helper.create_launch_config(
            request, group_name, launchRequest
        )
    ...
    try:
        ...
        if basic_cluster_info:
            ...
            placements = placements_helper.get_simplified_by_ids(
                request,
                account_id,
                basic_cluster_info["placement"],
                basic_cluster_info["provider"],
                basic_cluster_info["cellName"],
            )
...
def delete_asg(request, group_name):
    try:
        ...
        autoscaling_groups_helper.delete_autoscaling(
            request, group_name, detach_instances
        )
        content = render_to_string(
            "groups/deletion_loading.tmpl", {"group_name": group_name}
        )
    except:
        raise
    return HttpResponse(json.dumps(content), content_type="application/json")
...
def get_deleted_asg_status(request, group_name):
    try:
        ...
        return HttpResponse(
            json.dumps({"content": content, "status": asg_status}),
...
def update_asg_config(request, group_name):
    ...
    if asg_minsize < 0 or asg_maxsize < 0:
        ...
        raise TeletraanException(
            "Please put valid values (>=0) in Auto Scaling Group config's Min Size and Max Size fields"
        )
        ...
        if len(scheduled_actions) > 0:
            for action in scheduled_actions:
                ...
                if capacity is not None and (
                    capacity < asg_minsize or capacity > asg_maxsize
                ):
                    raise TeletraanException(
                        "Desired capacity in the scheduled scaling actions is invalid. Please update the scheduled actions to make sure all the desired capacities are within the limits of ASG's minimum capacity ({}) and maximum capacity ({})".format(
                            asg_minsize, asg_maxsize
                        )
                    )
...
def get_policy(request, group_name):
    ...
    if step_scaling_policy is not None:
        for step in step_scaling_policy["stepAdjustments"]:
            if (
                step["metricIntervalUpperBound"] is not None
                and float(step["metricIntervalUpperBound"]) <= 0
            ):
                scale_down_steps.append(
                    {
                        "upper_bound": float(step["metricIntervalUpperBound"]),
                        "adjustment": step["scalingAdjustment"],
                    }
                )
            if (
                step["metricIntervalLowerBound"] is not None
                and float(step["metricIntervalLowerBound"]) >= 0
            ):
                scale_up_steps.append(
                    {
                        "lower_bound": float(step["metricIntervalLowerBound"]),
                        "adjustment": step["scalingAdjustment"],
                    }
                )
        ...
        scale_down_steps_string = ", ".join(
            [str(step["upper_bound"]) for step in scale_down_steps]
        )
        scale_up_steps_string = ", ".join(
            [str(step["lower_bound"]) for step in scale_up_steps]
        )

        scale_down_adjustments_string = ", ".join(
            [str(step["adjustment"]) for step in scale_down_steps]
        )
        scale_up_adjustments_string = ", ".join(
            [str(step["adjustment"]) for step in scale_up_steps]
        )
        ...
        step_scaling_policy["scale_down_adjustments_string"] = (
            scale_down_adjustments_string
        )
        step_scaling_policy["scale_up_adjustments_string"] = scale_up_adjustments_string

    content = render_to_string(
        "groups/asg_policy.tmpl",
        {
            "group_name": group_name,
...
def build_target_scaling_policy(
    name, metric, target, instance_warmup, disable_scale_in
):
    policy = {}
    ...
    targetTrackingScalingConfiguration["predefinedMetricSpecification"] = (
        predefinedMetricSpecification
    )
...
def delete_policy(request, group_name, policy_name):
    try:
        autoscaling_groups_helper.delete_scaling_policy(
            request, group_name, policy_name
        )
...
def update_policy(request, group_name):
    try:
        ...
        if policyType == "target-tracking-scaling":
            ...
            if "content" in params:
                ...
                for name in policy_names.values():
                    ...
                    policy = build_target_scaling_policy(
                        name,
                        input[name + "_awsMetrics"],
                        input[name + "_target"],
                        input[name + "_instanceWarmup"],
                        disable_scale_in,
                    )
            ...
            scaling_policies["scaleupPolicies"].append(
                {
                    "scaleSize": make_int(params["scaleupSize"]),
                    "scalingType": params["scaleupType"],
                    "coolDown": make_int(params["scaleupCooldownTime"]),
                }
            )
            scaling_policies["scaledownPolicies"].append(
                {
                    "scaleSize": make_int(params["scaledownSize"]),
            ...
            if params["instanceWarmup"]:
                step_scaling_policy["instanceWarmup"] = (
                    int(params["instanceWarmup"]) * 60
                )

            step_scaling_policy["metricAggregationType"] = "Average"

            if params["scalingType"] == "PercentChangeInCapacity":
                step_scaling_policy["minAdjustmentMagnitude"] = params[
            ...
            if params["scaleUpSteps"]:
                ...
                scaleUpAdjustments = [
                    int(x) for x in params["scaleUpAdjustments"].split(",")
                ]
            ...
            if params["scaleDownSteps"]:
                ...
                scaleDownAdjustments = [
                    int(x) for x in params["scaleDownAdjustments"].split(",")
                ]
        ...
        autoscaling_groups_helper.put_scaling_policies(
            request, group_name, scaling_policies
        )
...
def _parse_metrics_configs(request, group_name):
    ...
    for key, value in page_data.items():
        ...
        if key.startswith("TELETRAAN_"):
            ...
            if page_data.get("schedule_{}".format(alarm_id)) or page_data.get(
                "capacity_{}".format(alarm_id)
            ):
                continue
            ...
            alarm_info["evaluationTime"] = int(
                page_data["evaluateTime_{}".format(alarm_id)][0]
            )
            alarm_info["threshold"] = float(
                page_data["threshold_{}".format(alarm_id)][0]
            )
...
def get_alarms(request, group_name):
    ...
    for alarm in alarms:
        ...
        if not alarm["fromAwsMetric"]:
            alarm["previewUrl"] = _construct_alarm_preview_url(
                alarm["metricSource"],
                alarm["comparator"],
                alarm["threshold"],
                alarm["evaluationTime"],
            )
        alarm["cwLink"] = (
            f"https://console.aws.amazon.com/cloudwatch/home?#alarmsV2:alarm/{alarm['groupName']}-alarm-{alarm['alarmId']}"
        )
...
def add_alarms(request, group_name):
    ...
    if len(alarm_info["scalingPolicies"]) == 0:
        ...
        messages.add_message(
            request,
            messages.ERROR,
            "Alarm could not be created because there was no {} policy.".format(
                policy_type
            ),
        )
        return redirect("/groups/{}/config/".format(group_name))
...
def get_group_info(request, group_name):
    try:
        ...
        group_size = (
            len(asg_host_names) + len(non_asg_host_names) + len(spot_asg_host_names)
        )
        ...
        return HttpResponse(
            json.dumps({"html": content}), content_type="application/json"
        )
    except Exception:
        log.error(traceback.format_exc())
...
def get_group_size(request, group_name):
    try:
        group_size_datum = autoscaling_metrics_helper.get_asg_size_metric(
            request, group_name, settings.DEFAULT_START_TIME
        )

        spot_group_size_datum = autoscaling_metrics_helper.get_asg_size_metric(
            request, "{}-spot".format(group_name), settings.DEFAULT_START_TIME
        )

        alarm_infos = autoscaling_groups_helper.get_alarms(request, group_name)
        spot_group_name = "{}-spot".format(group_name)
        spot_alarm_infos = autoscaling_groups_helper.get_alarms(
            request, spot_group_name
        )
        enable_policy = False
        ...
        if alarm_infos:
            ...
            for idx in range(len(alarm_infos)):
                ...
                if spot_alarm_infos:
                    alarm_infos[idx]["spotActionType"] = spot_alarm_infos[idx][
                        "actionType"
                    ]
                    alarm_infos[idx]["spotThreshold"] = spot_alarm_infos[idx][
                        "threshold"
                    ]
                ...
                if idx > 0 and metric_name == alarm_infos[idx - 1]["metricSource"]:
                    ...
                    if spot_alarm_infos:
                        alarm_infos[idx - 1]["spotActionType2"] = spot_alarm_infos[idx][
                            "actionType"
                        ]
                        alarm_infos[idx - 1]["spotThreshold2"] = spot_alarm_infos[idx][
                            "threshold"
                        ]
                    removeIdx.append(idx)
                else:
                    alarm_info["metric_datum"] = (
                        autoscaling_metrics_helper.get_metric_data(
                            request,
                            group_name,
                            metric_name,
...
def get_scaling_activities(request, group_name):
    try:
        scaling_activities = autoscaling_groups_helper.get_scaling_activities(
            request, group_name, 10, ""
        )
        content = render_to_string(
            "groups/scaling_details.tmpl",
            {
                "group_name": group_name,
...
class ScalingActivityView(View):
    def get(self, request, group_name):
        ...
        return render(
            request,
            "groups/scaling_activities.html",
            {
...
def get_config_history(request, group_name):
    ...
    excludedTypes = list(
        filter(None, request.GET.get("exclude", "").replace("%20", " ").split(","))
    )

    return render(
        request,
        "groups/group_config_history.html",
        {
            "group_name": group_name,
            "configs": configs,
            "pageIndex": index,
...
def get_config_comparison(request, group_name):
    ...
    return HttpResponse(
        json.dumps({"change1": change1, "change2": change2}),
        content_type="application/json",
    )
...
def show_config_comparison(request, group_name):
    ...
    return render(
        request,
        "groups/group_config_comparison_result.html",
        {
            "group_name": group_name,
...
class GroupConfigView(View):
    def get(self, request, group_name):
        ...
        if asg_cluster:
            ...
            curr_image = None
            raise TeletraanException(
                "Group does not exist. Please create capacity from the environments page."
            )

        pas_config = autoscaling_groups_helper.get_pas_config(request, group_name)
        return render(
            request,
            "groups/asg_config.html",
            {
...
class GroupDetailView(View):
    ...
    def get(self, request, group_name):
        ...
        return render(
            ...
            "groups/group_details.html",
            {
                ...
                "disallow_autoscaling": _disallow_autoscaling(curr_image),
                "teletraan_user_dashboard_url": self.generate_dashboard_url(
                    group_name, envs, group_info
                ),
            },
        )
    ...
    def generate_dashboard_url(self, group, envs, group_info):
        ...
        env_arg = "|".join(
            [f"{env.get('envName')}.{env.get('stageName')}" for env in envs]
        )
        params = {
...
def get_aws_settings(request):
    ...
    contents = render_to_string(
        "groups/get_ami.tmpl",
        {"aws_images": images, "curr_image_id": params["curr_image_id"]},
    )
    return HttpResponse(json.dumps(contents), content_type="application/json")
...
def get_sg_settings(request):
    ...
    contents = render_to_string(
        "groups/get_sg.tmpl", {"security_groups": sorted_sgs, "curr_sg": curr_sg}
    )
    return HttpResponse(json.dumps(contents), content_type="application/json")
...
def get_instance_type_settings(request):
    ...
    contents = render_to_string(
        "groups/get_hosttype.tmpl",
        {"instance_types": host_types, "curr_instance_type": curr_instance_type},
    )
    return HttpResponse(json.dumps(contents), content_type="application/json")
...
def get_subnets_settings(request):
    ...
    if curr_subnets:
        ...
        currSubnetArrays = []
    contents = render_to_string(
        "groups/get_subnets.tmpl",
        {"subnets": sorted_subnets, "currSubnetArrays": currSubnetArrays},
    )
    return HttpResponse(json.dumps(contents), content_type="application/json")
...
def add_instance(request, group_name):
    ...
    try:
        ...
        if "subnet" in params:
            ...
            if (
                "customSubnet" in params
                and "customPlacementGroup" in params
                and "placementGroup" in params
            ):
        ...
        if (
            asg_status == "ENABLED" or asg_status == "DISABLED"
        ) and not use_placement_group:
            ...
            if "customSubnet" in params:
                content += "\nNote: the subnet {} you selected was ignored.".format(
                    subnet
                )
            ...
            if not subnet:
                ...
                content = (
                    "Failed to launch hosts to group {}. Please choose subnets in"
                ...
                if placement_group is not None:
                    ...
                    host_ids = (
                        autoscaling_groups_helper.launch_hosts_with_placement_group(
                            request, group_name, num, subnet, placement_group
                        )
                    )
                else:
                    host_ids = autoscaling_groups_helper.launch_hosts(
                        request, group_name, num, subnet
                    )

                if len(host_ids) > 0:
                    content = (
                        "{} hosts have been launched to group {} (host ids: {})".format(
                    ...
                    content = (
                        "Failed to launch hosts to group {}. Please make sure the"
                        ' <a href="/groups/{}/config/">group config</a>'
...
def instance_action_in_asg(request, group_name):
    ...
    try:
        autoscaling_groups_helper.hosts_action_in_group(
            request, group_name, host_ids, action
        )
    except:
        log.error(traceback.format_exc())
        raise
...
def attach_instances(request, group_name):
    try:
        ...
        autoscaling_groups_helper.hosts_action_in_group(
            request, group_name, host_ids, "ATTACH"
        )
        return redirect("/groups/{}/".format(group_name))
    except Exception:
        log.error(traceback.format_exc())
...
def get_charts(request, group_name, type):
    if type == "az":
        return render(
            request,
            "groups/distribution_charts.tmpl",
            get_host_az_dist(request, group_name),
        )
        ...
        return render(
            request,
            "groups/distribution_charts.tmpl",
            get_host_ami_dist(request, group_name),
        )
        ...
        return render(
            request,
            "groups/distribution_charts.tmpl",
            get_host_type_dist(request, group_name),
        )


def get_host_az_dist(request, group_name):
    aws_owner_id = accounts_helper.get_aws_owner_id_for_cluster_name(
        request, group_name
    )
...
def get_host_ami_dist(request, group_name):
    ...
    if len(label_data_percentage) > 1 or (
        len(label_data_percentage) == 1 and label_data_percentage[0][0] != current_AMI
    ):
...
def get_health_check_details(request, id):
    ...
    if health_check_error:
        ...
        if IS_PINTEREST and PHOBOS_URL:
            ...
            cmdb = Brood()
            host_ip = cmdb.get_query(
                query="id:" + health_check["host_id"], fields="config.internal_address"
            )[0]["config.internal_address"]
...
def create_manually_health_check(request, group_name):
    ...
    autoscaling_groups_helper.create_health_check(
        request, group_name, health_check_info
    )
...
def add_scheduled_actions(request, group_name):
    ...
    asg_maxsize = int(
        asg_summary.get("maxSize", -1)
    )  # invalid value indicates no need to check

    if asg_minsize < 0 or asg_maxsize < 0:
        raise TeletraanException(
            "ASG's Min Size and Max Size have invalid values. Please update the ASG config to make sure Min Size and Max Size >= 0"
        )
    scheduled_action_capacity = int(params["capacity"])
    if (
        scheduled_action_capacity < asg_minsize
        or scheduled_action_capacity > asg_maxsize
    ):
        raise TeletraanException(
            "Invalid capacity: {}. Desired capacity must be within the limits of ASG's minimum capacity ({}) and maximum capacity ({}). Please change the value you input for Capacity.".format(
                params["capacity"], asg_minsize, asg_maxsize
            )
        )
...
def get_suspended_processes(request, group_name):
    suspended_processes = autoscaling_groups_helper.get_disabled_asg_actions(
        request, group_name
    )
    all_processes = autoscaling_groups_helper.get_available_scaling_process(
        request, group_name
    )
    all_processes.sort()
...
def suspend_process(request, group_name, process_name):
    ...
    autoscaling_groups_helper.update_scaling_process(
        request, group_name, update_request
    )
