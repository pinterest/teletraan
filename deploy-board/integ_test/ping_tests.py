...
class TestPings(unittest.TestCase):
    ...
    def ping(self, host=None, groups=None, envId=None, deployId=None, stage="UNKNOWN",
             status="UNKNOWN", error_code=0, error_message=None, deploy_alias=None, fail_count=0):
        ip = "8.8.8.8"
        reports = {}
    ...
    def test_agents_reset(self):
        ...
        agents_helper.reset_failed_agents(commons.REQUEST, self.envName, stageName, self.deploy['id'])
        # ping will start from beginning
        response = self.ping(self.host, self.groups, self.env['id'], self.deploy['id'],
                             "RESTARTING", "SUCCEEDED")
        self.assertEquals(response['opCode'], "DEPLOY")
