...
def main():
    ...
    parser.add_argument(
        "--deploy-stage", type=str, default="PRE_DOWNLOAD", help="deploy stage to set"
    )
    parser.add_argument(
        "--agent-status", type=str, default="SUCCEEDED", help="deploy status to set"
    )
    parser.add_argument("--error-code", type=int, default=0, help="Agent error code")
    parser.add_argument(
        "--error-message", type=str, default=None, help="Agent error message"
    )
    parser.add_argument("--deploy-alias", type=str, default=None, help="Deploy alias")
    parser.add_argument("--fail-count", type=int, default=0, help="Failed number")
    args = parser.parse_args()

    # first locate the env and deploy
    env = environs_helper.get_env_by_stage(
        commons.REQUEST, args.env_name, args.stage_name
    )
